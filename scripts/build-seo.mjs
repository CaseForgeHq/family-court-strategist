...
export const pages=[
 ...
 {file:'features.html',title:'What Case Forge Can Do | Features & Availability',description:'Explore the free Case Forge toolkit, verified facts, correction reviews and drift checks. See what works today and what is still in the desktop app preview.'},
...
export function buildSeo({directory=fileURLToPath(new URL('../website/',import.meta.url)),siteURL=process.env.SITE_URL,googleVerification=process.env.GOOGLE_SITE_VERIFICATION,bingVerification=process.env.BING_SITE_VERIFICATION}={}){
 ...
 for(const page of pages){
  ...
  if(canonical){
   ...
   if(page.article){
    const article={'@type':'Article',headline:page.title.replace(/ \| Case Forge$/,''),description:page.description,mainEntityOfPage:{'@id':webPage['@id']},author:{'@id':org['@id']},publisher:{'@id':org['@id']},datePublished:'2026-09-13',dateModified:page.file==='ai-document-review.html'?'2026-09-15':'2026-09-13',image:url(socialPath)};
 ...
 writeFileSync(join(directory,'sitemap.xml'),`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${base?indexed.map(p=>`  <url><loc>${escape(url(p.path??p.file))}</loc>${p.file==='features.html'||p.file==='ai-document-review.html'?'<lastmod>2026-09-15</lastmod>':p.article?'<lastmod>2026-09-13</lastmod>':''}</url>`).join('\n'):''}\n</urlset>\n`);
