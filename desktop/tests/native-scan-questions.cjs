...
const repo=resolve(__dirname,'../..'),out=join(repo,'output/scan-questions-native');
...
(async()=>{
  ...
  await win.webContents.executeJavaScript(`window.scanPreviewCard.open=true`);await capture('scan-report-compact',804,619,true);
  await win.webContents.executeJavaScript(`document.querySelector('.scan-card[open] [data-question-target="facts"]').click()`);
  assert.ok(await win.webContents.executeJavaScript(`document.querySelector('[data-section="facts"]').open`));
