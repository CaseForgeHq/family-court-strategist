...
async function showUpdateHandoff({ runtime, temp, executable, pid }) {
  ...
  const child = spawn(helper, [executable, String(pid), ready], { detached: true, stdio: 'ignore', windowsHide: true });
  let failed = false;
