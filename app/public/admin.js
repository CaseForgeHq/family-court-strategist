...
export function createAdmin({ desktop }) {
  ...
  function render() {
    ...
    $('.admin-options').innerHTML = state ? `<fieldset ${busy ? 'disabled' : ''}><legend>Choose a sample case</legend><div class="admin-sizes">${state.presets.map(item => `<label class="admin-size"><input type="radio" name="demo-size" value="${escape(item.id)}" ${selected === item.id ? 'checked' : ''}><span class="admin-size-name">${escape(item.label)}<span class="admin-radio" aria-hidden="true"></span></span><strong>${item.files}<span> files</span></strong><small>${({ review: 'Parenting + property', small: 'Quick look', medium: 'Everyday case', large: 'Stress test' })[item.id] || ''}</small></label>`).join('')}</div></fieldset>` : '';
    $('.admin-preview').innerHTML = preset?.id === 'review' ? '<div>Fictional parenting, expenses and property records</div><small>Known issues and ordinary differences. No pre-filled findings. Scan the files to test the review.</small>' : preset ? `<div>${preset.people} people <span>·</span> ${preset.events} events <span>·</span> ${preset.tasks} tasks</div><small>${preset.emailExports ?? Math.max(0, preset.files - 5)} email exports · ${preset.notes} notebook pages · ${preset.notes} journal notes<br>${preset.calendar} calendar entries · ${preset.evidence} evidence items · ${preset.patterns} themes</small>` : '';
