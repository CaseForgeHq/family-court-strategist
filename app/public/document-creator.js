...
export function createDocumentCreator({ api, getSession, desktop = () => window.strategistDesktop }) {
  ...
  async function mount(element, seed){
    ...
    // Shared chrome spans both tiled panes, keeping their top and bottom edges aligned.
    find('.document-columns').before(find('.document-view-tabs'));
    find('.document-columns').after(find('#document-status'));
