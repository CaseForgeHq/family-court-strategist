...
3. If Node.js 18 or newer and npm are available, use this command, replacing PACKAGE_URL with the absolute URL of `downloads/case-forge-setup-0.2.0.tgz` beside this document and TARGET with the user's chosen folder:
...
6. Verify that `CASE-FORGE.md`, `HOME.md`, `CASE-DETAILS.md`, `_system/attribution-standards.md`, `_system/verified-facts.md`, `_system/security-and-privacy.md`, `.case-forge/tools/facts-cli.mjs` and `_templates/` exist in the installed folder. Read `CASE-FORGE.md`. Tell the user where the folder is and what was added.
...
- CLI package: [case-forge-setup-0.2.0.tgz](downloads/case-forge-setup-0.2.0.tgz)
...
The current download includes case folders, eight templates, evidence and chronology guidance, digital-source attribution, privacy guidance and the verified-fact command. Read [features and availability](features.html) for the complete overview.

Verified facts require Node.js 18+ and explicit human review. The command records source references and exact wording, manages conflicts, renders canonical references and checks draft exports for drift. It does not automatically verify existing notes or update signed/filed documents. Without Node.js, the ordinary files remain usable, but the registry command cannot run.

For an existing case, preserve the case and its registry. Install into a separate folder and compare the new guidance and tools before updating; never overwrite case records with the empty template.
